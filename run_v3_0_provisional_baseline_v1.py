...
def main():
    ap = argparse.ArgumentParser(description='Generate provisional baseline v1 command logs: v3.0.36 RF-1 current-angle anchor + x8 smooth_window=40.')
    ap.add_argument('--output-dir', default='testdata/provisional_baseline_v1')
    ...
    _ensure_dir(out_dir)
    raw_log = os.path.join(out_dir, 'provisional_baseline_v1_raw_commands.jsonl')
    raw_report = os.path.join(out_dir, 'provisional_baseline_v1_generation_report.json')
    filtered_log = os.path.join(out_dir, 'provisional_baseline_v1_x8_sw40_commands.jsonl')
    smooth_report = os.path.join(out_dir, 'provisional_baseline_v1_resample_summary.json')
    manifest = os.path.join(out_dir, 'provisional_baseline_v1_manifest.json')
    ...
    _run(gen_cmd)
    # Capture the resampler JSON summary into a file while still using the same implementation path as manual operation.
    ...
    info = {
        'baseline_name': 'provisional_baseline_v1',
        'description': 'v3.0.36 RF-1 current-angle anchor pure legacy repeated roll; x8 resample; smooth_window=40; segment_key none.',
        'parameters': {
