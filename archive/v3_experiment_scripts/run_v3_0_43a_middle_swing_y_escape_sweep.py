...
"""v3.0.43A middle_swing_y_escape_sweep.

Purpose:
  Test whether a small lateral Y escape of the RF-3/RF-4 middle-pair
  lift/land targets can reduce the middle-leg proximity / interference
  suspicion without breaking the provisional baseline-v2 geometry.
...
def score_candidate(item, baseline, args):
    """Lower is better.  v3.0.43A prioritizes middle RF-3/RF-4 clearance."""
    ...
    # Middle-phase near counts are the main v3.0.43A target.
...
def gate_status(item, baseline, args):
    ...
    return {
        ...
        'note': 'v3.0.43A gate is for screening only. It checks middle RF-3/RF-4 proximity and basic non-regression; Gazebo strict replay remains mandatory.',
    }
...
def evaluate_case_isolated(args, case, label):
    ...
    generations are executed in one long-lived Python process.  v3.0.43A keeps
...
def main():
    ap = argparse.ArgumentParser(description='v3.0.43A middle_swing_y_escape_sweep: local RF-3/RF-4 Y escape test from provisional baseline v2.')
    ...
    ap.add_argument('--middle-swing-y-escapes', default='0.00,0.01,0.02,0.03,0.04')
    ...
    ap.add_argument('--middle-swing-y-escape-phases', default='rf3_rf4', help='Comma list: rf3_only,rf4_only,rf3_rf4')
    ...
    ap.add_argument('--output', default='testdata/v3_0_43a_middle_swing_y_escape_sweep.json')
    ap.add_argument('--candidate-output-dir', default='testdata/v3_0_43a_candidates')
    ...
    print('Evaluating v3.0.43A escape=0 baseline...', file=sys.stderr)
    worker_dir = tempfile.mkdtemp(prefix='v3_0_43a_', dir=os.path.dirname(args.output) or None)
    ...
    for idx, case in enumerate(cases):
        print('Evaluating Y escape case %d/%d: %s' % (idx + 1, len(cases), make_case_key(case)), file=sys.stderr)
    ...
    out = {
        'version_note': 'v3.0.43A middle_swing_y_escape_sweep. Minimum-difference experiment from provisional baseline v2. It only changes RF-3/RF-4 middle-pair Y targets when explicitly enabled. escape=0/mode=none is the baseline reproduction check.',
        'roadmap_position': {
            'name': 'v3.0.43A middle_swing_y_escape_sweep',
            'purpose': 'Check whether lateral Y escape during RF-3/RF-4 middle-pair lift/land improves middle-leg proximity / interference suspicion without losing provisional baseline-v2 constraints.',
            'not_a_refactor': True,
        ...
        'sweep_parameters': {
            ...
            'score_focus': 'middle RF-3/RF-4 inter-leg and joint-housing clearance, with second-joint/floor/smoothness non-regression gates',
        },
